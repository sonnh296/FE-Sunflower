<template>
  ...
  >
    <span class="text-surface-500 dark:text-surface-400 block mb-8"
      >Time cannot more than 1.5 hours.</span
    >
...
import { changeTimeToSeconds, getCurrentTime, isBefore } from '@/utils/timeUtil'
import BookingDialog from '../dialogs/BookingDialog.vue'
import { getDateFromDate } from '@/utils/dateUtil'
import { DATE_FORMAT_DD_MM_YYYY } from '@/utils/dateUtil'
        ...
        !bookingInThisDate?.some((b) => {
          ...
          return (
            (bookingStartTimeSeconds > slotStartTimeSeconds &&
              bookingStartTimeSeconds < slotEndTimeSeconds) ||
            (bookingEndTimeSeconds > slotStartTimeSeconds &&
        ...
        !inactiveTimeThisDateInside.some((i) => {
          ...
          return (
            (inactiveStartTimeSeconds > slotStartTimeSeconds &&
              inactiveStartTimeSeconds < slotEndTimeSeconds) ||
            (inactiveEndTimeSeconds > slotStartTimeSeconds &&
...
onMounted(() => {
  ...
  calendar.value.on('clickEvent', (data: EventInfo) => {
    ...
    switch (data.event?.title) {
      case 'Trống': {
        startTimeBooking.value = data.event.start.substring(11, 16)
        endTimeBooking.value = data.event.end.substring(11, 16)
        console.log(startTimeBooking.value)
